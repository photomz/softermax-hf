"""
Softermax implementation of llama. 

Much of the code is adapted from huggingface llama repo here:
...
class SofterLlamaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: SofterLlamaConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class SofterLlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: SofterLlamaConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        # we have to override whatever self-attention layer is initialised with our own
        ...
        self.self_attn = SofterLlamaAttention(config=config, layer_idx=layer_idx)


class SofterLlamaPreTrainedModel(LlamaPreTrainedModel):
    config_class = SofterLlamaConfig
    _no_split_modules = ["SofterLlamaDecoderLayer"]
    _supports_flash_attn_2 = False
    _supports_sdpa = False
    _supports_cache_class = True


class SofterLlamaModel(SofterLlamaPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`SofterLlamaDecoderLayer`]
...
class SofterLlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = SofterLlamaModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, SofterLlamaForCausalLM

        >>> model = SofterLlamaForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
